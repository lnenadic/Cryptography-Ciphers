    ...
    if (isLetter(c)) {
      if (isUpperCase(c)) {
        result += String.fromCharCode(
          ((c.charCodeAt(0) + key.toString().toUpperCase().charCodeAt(j) - 2 * 65) % 26) +
            65
        ); // A: 65
      } else {
        result += String.fromCharCode(
          ((c.charCodeAt(0) + key.toString().toLowerCase().charCodeAt(j) - 2 * 97) % 26) +
            97
        ); // a: 97
      }
    } else {
...
export function decrypt(message, key) {
  ...
  for (let i = 0, j = 0; i < message.length; i++) {
    ...
    if (isLetter(c)) {
      if (isUpperCase(c)) {
        result += String.fromCharCode(
          90 - ((25 - (c.charCodeAt(0) - key.toString().toUpperCase().charCodeAt(j))) % 26)
        );
      } else {
        result += String.fromCharCode(
          122 -
            ((25 - (c.charCodeAt(0) - key.toString().toLowerCase().charCodeAt(j))) % 26)
        );
      }
    } else {
      result += c;
